...
function showSetupID()
{
	...
	function deleteDescription()
	{
		...
		else
		{
			if(setup == "Location")
			{
				var deleteItem = confirm("<bean:message key='BzComposer.configuration.customerinvoice.deleteselectedrecord'/>");
				if(deleteItem)
				{
	...
	function updateExistingJobCategory()
	{
	    ...
	    else
	    {
			$("#jobCategory option").each(function() 
			{
                ...
                {
                    itemExists= true;
                    alert('<bean:message key="BzComposer.configuration.customerinvoice.samejobcantupdatetolist"/>');
                    event.preventDefault();
	...
	function EditServiceBillInfo()
	{
		...
		{
			alert("<bean:message key='BzComposer.configuration.customerinvoice.enterrecurringservicecategory'/>");
		}
		else
		{
        ...
        else
        {
            ...
            $("#refundReasonSel option").each(function() {
          	...
          	if (!itemExists) 
          	{
				...
				//debugger
				document.getElementById('tabid').value="addRefundReason";
				document.forms[0].action = "Configuration.do?tabid=config6&&tab=tr6";
				document.forms[0].submit();
