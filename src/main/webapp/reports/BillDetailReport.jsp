...
<title>${sessionScope.user}-<bean:message key="BzComposer.Report.Banking&Accounting.BillDetail"/></title>
</head>
<body>

<html:form action="BankingAccounting.do?tabid=BillDetail" method="post">
...
<div id="headerBar">
											...
											<h6 style="text-align: center; color: blue;"
												id="headerBarValue">
												<bean:message key="BzComposer.Report.Banking&Accounting.BillDetail" />
											</h6>
										</div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<thead>
			<tr>
					<th ><bean:message key="BzComposer.Report.Banking&Accounting.BillNumber" /></th>
					<th><bean:message key="BzComposer.vendor.vendorsName" /></th>
					<th ><bean:message key="BzComposer.Invoice.Date.DateAdded" /></th>
					<th ><bean:message key="BizComposer.Configuration.Reminders.DueDate" /></th>
					<th ><bean:message key="BzComposer.Employee.Memo" /></th>
					<th ><bean:message key="BzComposer.Report.Lists.PaymentStatus" /></th>
					<th ><bean:message key="BzComposer.accounting.CheckNo" /></th>
					<th ><bean:message key="BzComposer.Report.Amount" /></th>
			</tr>
		</thead>
		<tbody>
				<logic:present name="billDetail" scope="request">		
				<logic:notEmpty name="billDetail" scope="request">

					<bean:size id="SalesListSize" name="billDetail" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="billDetail" id="objList" indexId="ndx">
						<tr>
							...
							<td><bean:write name="objList"
								property="vendorName" />
							<td><bean:write name="objList" 	property="date" /></td>
							<td><bean:write name="objList" property="dueDate" /></td>
							<td><bean:write name="objList" 	property="memo" /></td>
							<td><bean:write name="objList" 	property="paymentStatus" /></td>
							<td><bean:write name="objList" 	property="chekcno" /></td>
...
function search()
{
	document.forms[0].action = "BankingAccounting.do?tabid=BillDetail";
