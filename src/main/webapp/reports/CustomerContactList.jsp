...
<title>${sessionScope.user} - <bean:message key="BzComposer.Report.CustomerContactList"/></title>
</head>
<body>

<html:form action="Customer.do?tabid=CustomerContactList" method="post">
	...
	<div id="ddcolortabsline">&nbsp;</div>
		...
		<bean:message key="BzComposer.Report.CustomerContactList" /></span> --%>
		...
		<div id="printContent">
			<div id="headerBar">
				...
				<h6 style="text-align: center; color: blue;" id="headerBarValue">
					<bean:message key="BzComposer.reportcenter.listing.customercontactlist" />
				</h6>
			</div>
			<div id="table-negotiations"
				...
				<table id="exportPd" class="tabla-customListOds" border="1">
					...
					<thead>
						<tr>
							<th class="emblem"><bean:message key="BzComposer.configuration.tab.customer" /></th>
							<th><bean:message key="BzCompoer.report.customercontract.fullname" /></th>
							<th><bean:message key="BzComposer.report.vendorcontactlist.billto" /></th>
							<th><bean:message key="BzComposer.global.phone" /></th>
							<th><bean:message key="BzComposer.global.email" /></th>
						</tr>
					</thead>
					<tbody>
						<logic:present name="customerContactList" scope="request">
							<logic:notEmpty name="customerContactList" scope="request">

								<bean:size id="SalesListSize" name="customerContactList" />
								<input type="hidden" name="sListSize" id="lSize"
									value='<bean:write name="SalesListSize" />'>

								<logic:iterate name="customerContactList" id="objList"
									indexId="ndx">
									<tr>
...
function search()
{
	document.forms[0].action = "Customer.do?tabid=CustomerContactList";
