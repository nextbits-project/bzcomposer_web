...
<title>${sessionScope.user}-<bean:message key="BzComposer.Report.Banking&Accounting.CheckDetail"/></title>
</head>
<body>

<html:form action="BankingAccounting.do?tabid=CheckDetail" method="post">
...
<div id="printContent">	
	...
	<div id="headerBar">
											...
											<h6 style="text-align: center; color: blue;"
												id="headerBarValue">
												<bean:message key="BzComposer.Report.Banking&Accounting.CheckDetail" />
											</h6>
										</div>
...
<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<tbody>
				<logic:present name="checkDetail" scope="request">		
				<logic:notEmpty name="checkDetail" scope="request">

					<bean:size id="SalesListSize" name="checkDetail" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="checkDetail" id="objList" indexId="ndx">
						<tr>
...
function search()
{
	document.forms[0].action = "BankingAccounting.do?tabid=CheckDetail";
