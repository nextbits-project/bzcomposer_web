...
<title>${sessionScope.user}-<bean:message key="BzComposer.Report.Banking&Accounting.AccountPayable"/></title>
</head>
<body>

	<html:form action="Customer.do?tabid=AccountPayable" method="post">
			...
			<div id="ddcolortabsline">&nbsp;</div>
								...
								<div id="headerBar">
											...
											<h6 style="text-align: center; color: blue;"
												id="headerBarValue">
												<bean:message key="BzComposer.Report.Banking&Accounting.AccountPayable" />
											</h6>
										</div>
											...
											style="overflow: auto; height: 400; text-align: center;">
												<table class="tabla-customListOds" id="exportPd" border="1">
													...
													<thead>
														<tr>
															<th><bean:message
																	key="BzComposer.Report.Rcolumn.Ref" /></th>
															<th><bean:message key="BzComposer.accounting.Date" /></th>
															<th><bean:message
																	key="BzComposer.Report.AllInvoice.ClientVendor" /></th>
															<th><bean:message key="BzComposer.Report.Amount" /></th>
															<th><bean:message
																	key="BzComposer.accounting.Balance" /></th>
														</tr>
													</thead>
													<tbody>
														<logic:present name="acPayList" scope="request">
															<logic:notEmpty name="acPayList" scope="request">

																<bean:size id="SalesListSize" name="acPayList" />
																<input type="hidden" name="sListSize" id="lSize"
																	value='<bean:write name="SalesListSize" />'>

																<logic:iterate name="acPayList" id="objList"
																	indexId="ndx">
																	<tr>
...
function search()
{
	document.forms[0].action = "Customer.do?tabid=AccountPayable";
