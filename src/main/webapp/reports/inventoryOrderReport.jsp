...
<title>${sessionScope.user} - <bean:message key="BzComposer.InventoryOrderReport"/></title>
</head>
<body>

<html:form action="Item.do?tabid=InvOrderReport" method="post">
	...
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.InventoryOrderReport"/></h6>
	</div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<thead>
			<tr>
				...
				<th><bean:message key="BzComposer.Item.SalePrice" /></th>
				<th><bean:message key="BzComposer.purchase.PONumPrefix" /></th>
				
			</tr>
		</thead>
		<tbody>
			<logic:present name="invOrderReport" scope="request">
				<logic:notEmpty name="invOrderReport" scope="request">

					<bean:size id="SalesListSize" name="invOrderReport" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="invOrderReport" id="objList" indexId="ndx">
						<tr >
...
function search()
{
	document.forms[0].action = "Item.do?tabid=InvOrderReport";
