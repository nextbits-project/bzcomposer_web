...
<title>${sessionScope.user} - <bean:message key="BzComposer.Report.Profit&Budget.ProfitLossByItem"/></title>
</head>
<body>

<html:form action="Item.do?tabid=ProfitLossByItem" method="post">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.Report.Profit&Budget.ProfitLossByItem"/></h6>
	</div>
	...
	<table class="tabla-customListOds" cellspacing="0" id="exportPd">
		...
		<thead>
			<tr>
					<th style="padding-left: 50px"><bean:message key="BzComposer.Invoice.ItemID" /></th>
					<th style="padding-left: 50px"><bean:message key="BzComposer.Report.ReservedInventoryList.InventoryCode" /></th>
					<th style="padding-left: 50px"><bean:message key="BzComposer.Item.SalePriceEachItem" /></th>
					<th style="padding-left: 50px"><bean:message key="BzComposer.Item.TotalSalePrice" /></th>
			</tr>
		</thead>
		<tbody>
				<logic:present name="profitLossByItem" scope="request">		
				<logic:notEmpty name="profitLossByItem" scope="request">

					<bean:size id="SalesListSize" name="profitLossByItem" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="profitLossByItem" />'>
					<logic:iterate name="profitLossByItem" id="objList" indexId="ndx">
						<tr>
						<td style="padding-left: 50px"><bean:write name="objList" property="inventoryId"/></td>
						<td style="padding-left: 50px"><bean:write name="objList" property="inventoryCode" /></td>
						<td style="padding-left: 50px"><bean:write name="objList" property="salePrice" /></td>
						<td style="padding-left: 50px"><bean:write name="objList" 	property="totalSaleprice" /></td>
						</tr>
						
					</logic:iterate>
					<tr>
						<td style="color: blue;padding-left: 50px"><b><bean:message key="BzComposer.Invoice.Total"/></b></td>
						<td></td>
						<td></td>
						<td style="color: blue;padding-left: 50px"><bean:write name="TotalGrossprofit"/></td>
					</tr> 
				</logic:notEmpty>
			</logic:present>
		</tbody>
