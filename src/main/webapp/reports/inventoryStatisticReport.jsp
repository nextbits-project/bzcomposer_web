...
<title>${sessionScope.user} - <bean:message key="BzComposer.Inventory.CurrentInventoryStatictics"/></title>
</head>
<body>

<html:form action="Item.do?tabid=InvStatistic" method="post">
	...
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.Inventory.CurrentInventoryStatictics"/></h6>
	</div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
				<th><bean:message key="BzComposer.sales.Item" /></th>
				<th><bean:message key="BzComposer.Item.ItemCode" /></th>
				<th><bean:message key="BzComposer.Invoice.ItemName" /></th>
				<th><bean:message key="BzComposer.Item.Location" /></th>
				
			</tr>
		</thead>
		<tbody>
			<logic:present name="invStatistics" scope="request">
				<logic:notEmpty name="invStatistics" scope="request">

					<bean:size id="SalesListSize" name="invStatistics" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="invStatistics" id="objList" indexId="ndx">
						<tr >
...
function search()
{
	document.forms[0].action = "Item.do?tabid=InvStatistic";
