...
<title>${sessionScope.user}-<bean:message key="BzComposer.Report.Banking&Accounting.DepositDetail"/></title>
</head>
<body>

<html:form action="BankingAccounting.do?tabid=DepositDetail" method="post">
...
<div id="headerBar">
											...
											<h6 style="text-align: center; color: blue;"
												id="headerBarValue">
												<bean:message key="BzComposer.Report.Banking&Accounting.DepositDetail" />
											</h6>
										</div>
...
<div id="table-negotiations"
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		<thead>
			<tr>
					<th ><bean:message key="BzComposer.Report.Banking&Accounting.AccountName" /></th>
					<th><bean:message key="BzComposer.Invoice.Date.DateAdded" /></th>
					<th ><bean:message key="BzComposer.InvoiceId" /></th>
					<th ><bean:message key="BzComposer.ClientId" /></th>
					<th ><bean:message key="BzComposer.Report.Amount" /></th>
			</tr>
		</thead>
		<tbody>
				<logic:present name="depositDetail" scope="request">		
				<logic:notEmpty name="depositDetail" scope="request">

					<bean:size id="SalesListSize" name="depositDetail" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="depositDetail" id="objList" indexId="ndx">
						<tr>
							...
							<td><bean:write name="objList"
								property="date" />
							<td><bean:write name="objList" 	property="invoiceId" /></td>
							<td><bean:write name="objList" property="clientVendorId" /></td>
...
function search()
{
	document.forms[0].action = "BankingAccounting.do?tabid=DepositDetail";
