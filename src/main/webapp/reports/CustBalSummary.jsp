...
<title>${sessionScope.user} - <bean:message key="BzComposer.Report.CustomerList.BalanceSummary"/></title>
</head>
<body>

<html:form action="Customer.do?tabid=CustomerBalanceSummary" method="post">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.Report.CustomerList.BalanceSummary"/></h6>
	</div>
	<div id="table-negotiations"
		...
		<thead>
			<tr>
					...
					<th><bean:message key="BzComposer.Report.TotalBalance" /></th> 
			</tr>
		</thead>

		<tbody>
				<logic:present name="balanceSummanyList" scope="request">
				<logic:notEmpty name="balanceSummanyList" scope="request">
<%-- 					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="catList" />'>
 --%>				<logic:iterate name="balanceSummanyList" id="objList" indexId="ndx">
						<tr>
						<td><bean:write name="objList" property="fullName" /></td>
...
function search()
{
	document.forms[0].action = "Customer.do?tabid=CustomerBalanceSummary";
