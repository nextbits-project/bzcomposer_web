...
<title>${sessionScope.user} - <bean:message key="BzComposer.reportcenter.listing.transactionlistbycustomer"/></title>
</head>
<body>

<html:form action="Customer.do?tabid=CustomerTransactionList" method="post">
...
<div id="printContent">	
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.Report.Transactionlist.ByCustomer"/></h6>
	</div>
	<div id="table-negotiations"
		...
		<thead>
			<tr>
			<th><bean:message key="BzComposer.Companyinformation.Customer" /></th>
			<th><bean:message key="BzComposer.Invoice.Date" /></th> 
			<th><bean:message key="BzComposer.Invoice.Num" /></th> 
			<th><bean:message key="BzComposer.Employee.Memo" /></th>
			<th><bean:message key="BzComposer.Employee.Amount" /></th>
			</tr>
		</thead>

		<tbody>
				<logic:present name="customerTransactionList" scope="request">
				<logic:notEmpty name="customerTransactionList" scope="request">
<%-- 					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="catList" />'>
 --%>				<logic:iterate name="customerTransactionList" id="objList" indexId="ndx">
						<tr>
						<td style="font-weight: bold;"><bean:write name="objList" property="cname" /></td>
						  	<td><bean:write name="objList" property="date" /></td>
						  	<td><bean:write name="objList" property="orderNo" /></td>
						  	<td><bean:write name="objList" property="memo" /></td>
						  	<td><bean:write name="objList" property="total" /></td>
						</tr>
					</logic:iterate>
				</logic:notEmpty>
			</logic:present>
...
function search()
{
	document.forms[0].action = "Customer.do?tabid=CustomerTransactionList";
