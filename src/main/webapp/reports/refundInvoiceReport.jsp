	...
	<div class="report-form-underheader" style="width: auto;">
		<table>
			<tr>
	    		...
	    		<td>
	    			<label style="padding-left: 15px"><bean:message key="BzComposer.sales.FromDate"/></label>
    			</td>
	    		<td>
	    		...
	    		<td>
	    			<label style="padding-left: 15px"><bean:message key="BzComposer.sales.to"/></label>
    			</td>
	    		<td>
	    		...
	    		<td>
	    			<label style="padding-right: 10px;padding-left: 15px"><bean:message key="BzComposer.Report.labelSortBy"/></label>
    			</td>
	    		<td>
	    	    ...
	    	    <td>
	    	    	<input type="button" value='<bean:message key="BzComposer.sales.Search"/>' class="formbutton mar" style="margin-left: 195px" 
	    	    	onclick="search()">
    	    	</td>
	    	</tr>
		</table>
	...
	<div id="printContent">	
		...
		<div id="table-negotiations" style="overflow:auto;height:400; text-align: center;">
			<table class="tabla-customListOds" cellspacing="0" id="exportPd" border="1">
				<thead>
					<tr>
						<th><bean:message key="BzComposer.Invoice.InvoiceNo"/></th>
						<th><bean:message key="BzComposer.Invoice.Date"/></th>
						<th><bean:message key="BzComposer.Report.labelSortBy.client/vendor"/></th>
						<th><bean:message key="Bzcomposer.Report.refundedBy"/></th>
						<th><bean:message key="BzComposer.Report.labelSortBy.amount"/></th>
					</tr>
				</thead>
				<tbody>
					<logic:present name="refundInvoice" scope="request">
					<logic:notEmpty name="refundInvoice" scope="request">
					<%--<input type="hidden" name="sListSize" id="lSize" value='<bean:write name="catList" />'>--%>				
						<logic:iterate name="refundInvoice" id="objList" indexId="ndx">
							<tr>
								<td>
									<bean:write name="objList" property="invoiceID" />
								</td>
								<td>
									<bean:write name="objList" property="dateAdded" />
								</td>	
								<td>
									<bean:write name="objList" property="cvName" />
								</td>							
						    	<td>
						    		<bean:write name="objList" property="firstName" />&nbsp;<bean:write name="objList" property="lastName" />
								</td>
								<td>
									<bean:write name="objList" property="amount"/>
								</td>
							</tr>
						</logic:iterate>
					</logic:notEmpty>
...
function search()
{
	document.forms[0].action = "SalesBord.do?tabid=SalesRID&ilist=3";
