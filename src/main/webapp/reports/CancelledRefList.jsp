...
<title><bean:message key="BzComposer.Report.CancelledPurchase"/></title>
</head>
<body>

<html:form action="PurchaseBoard.do?tabid=CancelledPurREfBill" method="post">
...
<div id="headerBar">
											...
											<h6 style="text-align: center; color: blue;"
												id="headerBarValue">
												<bean:message key="BzComposer.Report.CancelledPurchase" />
											</h6>
										</div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<thead>
			<tr>
					<th ><bean:message key="BzComposer.purchase.RefPONumPrefix" /></th>
					<th><bean:message key="BzComposer.Report.ReservedInventoryList.Date" /></th>
					<th ><bean:message key="BzComposer.Valuation.InventoryValuation.Detail.Customer" /></th>
					<th ><bean:message key="BzComposer.Vendor.Rep" /></th>
					<th ><bean:message key="BzComposer.Invoice.Amt" /></th>
					<th ><bean:message key="BzComposer.accounting.PaymentAmount" /></th>
					<th ><bean:message key="BzComposer.accounting.Balance" /></th>
					</tr>
		</thead>
		<tbody>
				<logic:present name="cancelledPuBillRef" scope="request">		
				<logic:notEmpty name="cancelledPuBillRef" scope="request">

					<bean:size id="SalesListSize" name="cancelledPuBillRef" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="cancelledPuBillRef" id="objList" indexId="ndx">
						<tr>
							...
							<td><bean:write name="objList" 	property="cvName" /></td>
							<td><bean:write name="objList" property="repname" /></td>
							<td><bean:write name="objList" 	property="total" /></td>
							<td><bean:write name="objList" 	property="paidAmount" /></td>
							<td><bean:write name="objList" 	property="balance" /></td>
...
function search()
{
	document.forms[0].action = "PurchaseBoard.do?tabid=CancelledPurREfBill";
