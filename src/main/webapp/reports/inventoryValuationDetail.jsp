...
<title>${sessionScope.user} - <bean:message key="BzComposer.Valuation.InventoryValuation.Detail"/></title>
</head>
<body>

<html:form action="Item.do?tabid=InvValDetail" method="post">
	...
	<div id="headerBar">
	      ...
	      <h6 style="text-align: center;color: blue;" id="headerBarValue"><bean:message key="BzComposer.Valuation.InventoryValuation.Detail"/></h6>
	</div>
	...
	<table class="tabla-customListOds" id="exportPd" border="1">
		...
		<thead>
			<tr>
				<th><bean:message key="BzComposer.Report.ReservedInventoryList.Category" /></th>
				<th><bean:message key="BzComposer.Valuation.InventoryValuation.Detail.Customer" /></th>
				<th><bean:message key="BzComposer.Report.vendor.BalanceSummary.Type" /></th>
				<th><bean:message key="BzComposer.Invoice.Date.DateAdded" /></th>
				<th><bean:message key="BzComposer.Invoice.Qty" /></th>
			</tr>
		</thead>
		<tbody>
			<logic:present name="getInvValDetail" scope="request">
				<logic:notEmpty name="getInvValDetail" scope="request">

					<bean:size id="SalesListSize" name="getInvValDetail" />
					<input type="hidden" name="sListSize" id="lSize"
						value='<bean:write name="SalesListSize" />'>

					<logic:iterate name="getInvValDetail" id="objList" indexId="ndx">
						<tr >
					...
					<%-- <logic:iterate name="invValSummaryList" id="objList" indexId="ndx" >
					 <tr>
					   <td align="left">Total</td>
					   <td></td>
					 	<td align="left"><bean:write name="objList"
								property="totalBal" /></td>
...
function search()
{
	document.forms[0].action = "Item.do?tabid=InvValDetail";
