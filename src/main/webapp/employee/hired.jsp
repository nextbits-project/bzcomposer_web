...
<title><bean:message key="BzComposer.hiredemployeelisttitle"/></title>
<script>
...
function terminateEmployee(Id)
{
	...
	/* var res=confirm("Do you want to Terminate Employee?");
	if(res==true)
	{
	...
	$("#showTerminatedEmployeeDialog").dialog({
		resizable: false,
	    height: 200,
	    width: 400,
	    modal: true,
	    buttons: {
			"<bean:message key='BzComposer.global.ok'/>": function () {
	        	...
	        	/* document.forms[0].action = "manageemployee.do?act=terminate&eid="+Id;
	    		document.forms[0].submit(); */
	    		window.location = "manageemployee.do?act=terminate&eid="+Id;
			},
    		...
    		<table  id="hireEmployeeList" class="tabla-listados" cellpadding="0" cellspacing="0">		
				...
				<tbody>
					...
					<logic:notPresent name="empList" scope="request">
						<div style="min-Height:800;"><bean:message key="BzComposer.employee.norecordsfound"/></div>
					</logic:notPresent>
				</div>
			</div>
		</div>
...
<form><input type="hidden" name="eid" id="eid"> <input type="hidden"
	name="act" id="act" value="terminate"> <input type="hidden" name="type"
	id="type" value="hired"></form>
</body>
</html>
<!-- Dialog box used in sales order page -->
<div id="showTerminatedEmployeeDialog" style="display:none;">
	<p><bean:message key="BzComposer.employee.terminateselectedemployee"/></p>
</div>
<script>
