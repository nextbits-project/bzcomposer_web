...
<title><bean:message key="BzComposer.coupondesigntitle"/></title>
</head>
<style>
.div
{
...
<div class="couponDesign">
	<div class="box-content">
		<div class="box">
			<label class="form-group">
				<bean:message key="BzComposer.coupondesign.textstamp"/>
				<!-- Text Stamp --> 
				<div>
			...
			<font id="resultText" style="font-size:20px;">
				<bean:message key="BzComposer.coupondesign.sometext"/>
				<!-- Some Text -->
			</font><br>
			<button class="btn btn-default"  id="stampBtn" name="stampBtn">
				<bean:message key="BzComposer.coupondesign.stamptext"/>
				<!-- Stamp Text -->
			</button>
			<button class="btn btn-default" onclick="showDialog()">
				<bean:message key="BzComposer.coupondesign.font"/>
				<!-- Font -->
			</button>
			<button class="btn btn-default" onclick="showDialog1()">
				<bean:message key="BzComposer.coupondesign.color"/>
				<!-- Color -->
			</button>
		</div>
		<div class="box">
			<label class="form-group">
				<bean:message key="BzComposer.coupondesign.barcodestamp"/>
				<!-- Barcode Stamp --> 
				<div>
			...
			<b>
				<bean:message key="BzComposer.coupondesign.type"/>
				<!-- Type: -->
			</b>
			<input type="radio" name="btype" id="code93" value="code93">
			<label for="code93">
				<bean:message key="BzComposer.coupondesign.code93"/>
				<!-- code 93 -->
			</label>
			&nbsp;&nbsp;
			<input type="radio" name="btype" id="code128" value="code128">
			<label for="code128">
				<bean:message key="BzComposer.coupondesign.code128"/>
				<!-- code 128 -->
			</label>
			<br/>
			<input type="radio" name="btype" id="datamatrix" value="datamatrix">
			<label for="datamatrix">
				<bean:message key="BzComposer.coupondesign.datamatrix"/>
				<!-- Data Matrix -->
			</label>
			<br>
			<br>
			<label for="title">
				<b>
					<bean:message key="BzComposer.coupondesign.format"/>
					<!-- Format: -->
				</b>
			</label>
			<input type="radio" id="css" name="renderer" value="css" checked="checked">
			<label for="css">
				<bean:message key="BzComposer.coupondesign.css"/>
				<!-- CSS -->
			</label>
			<br>
			<button class="btn btn-default" id="barcodeBtn" name="barcodeBtn">
				<bean:message key="BzComposer.coupondesign.stampbarcode"/>
				<!-- Stamp Barcode -->
			</button>
		</div>
	</div>
	<p>
		<bean:message key="BzComposer.coupondesign.coupondesign"/>
		<!-- Coupon size: 4" x 2.5" -->
	</p>
	</div>
	<div></div><div></div>
	...
	<a href="#" id="ExportToBMP" name="ExportToBMP">
		<button class="btn btn-default">
			<bean:message key="BzComposer.coupondesign.exportbmpimage"/>
			<!-- Export to BMP Image -->
		</button>
	</a>
	<button class="btn btn-default" id="delText">
		<bean:message key="BzComposer.coupondesign.deleteselection"/>
		<!-- Delete Selection -->
	</button>
	<button class="btn btn-default" onclick="Clear()">
		<bean:message key="BzComposer.coupondesign.clear"/>
		<!-- Clear -->
	</button>
	</div>
	<div id="dialog" style="display:none;">
		...
		<br><br>
  		<b>
  			<bean:message key="BzComposer.coupondesign.fontstyle"/>:
  			<!-- Font Style: -->
		</b>
		<select id="FontStyle" name="FontStyle">
  		...
  		<br><br>
    	<b>
    		<bean:message key="BzComposer.coupondesign.fontsize"/>:
    		<!-- Font Size: -->
   		</b>
  ...
  <div id="dialog1" style="display:none;">
    	<b>
    		<bean:message key="BzComposer.coupondesign.fontcolor"/>:
    		<!-- Font Color: -->
   		</b>
