...
public class AccountingDAO {
	...
	/**
	 * This method takes no argument. This method collect all the account
	 * information in a manner so that we can display all of the accounts in the
	 * tree structure in the JSP page.
	 * 
	 * @return ArrayList
	 */
	...
	public ArrayList getCheckDetailList(String cId, HttpServletRequest requset, String datesCombo, String fromDate,
			String toDate, String sortBy, AccountDto form) {
		Connection con = null;
		...
		String sql = "" + "SELECT inv.checknumber, " + "       date_format(inv.DateAdded,'%m-%d-%Y') AS DateFormat, "
				+ "       inv.invoiceid, " + "       inv.clientvendorid, " + "       inv.amount, "
				+ "       inv.accountid, " + "       NAME " + "FROM   bca_payment AS inv, "
				+ "       bca_account AS acc " + "WHERE  acc.accountid = inv.accountid "
				+ "       AND acc.companyid = '" + cId + "'" + "       AND inv.deleted = 0 "
				+ "       AND inv.checknumber > '0'" + dateBetween;
	...
	public ArrayList getDepositDetailReport(String cId, HttpServletRequest request, String datesCombo, String fromDate,
			String toDate, String sortBy, AccountDto form) {
		Connection con = null;
		SQLExecutor db = new SQLExecutor();
		Statement stmt = null;
		...
		String sql = "SELECT date_format(inv.DateAdded,'%m-%d-%Y')AS DateFormat, " + "       inv.invoiceid, "
				+ "       inv.clientvendorid, " + "       inv.amount, " + "       inv.accountid, "
				+ "       inv.checknumber, " + "       NAME " + "FROM   bca_payment AS inv "
				+ "       INNER JOIN bca_account AS acc " + "               ON inv.payeeid = acc.accountid "
				+ "WHERE  acc.companyid = '" + cId + "'" + "       AND acc.accttypeid = 2 "
				+ "       AND inv.deleted = 0" + dateBetween;
	...
	public ArrayList getBillDetailReport(String cId, HttpServletRequest request, String datesCombo, String fromDate,
			String toDate, String sortBy, AccountDto form) {
		Connection con = null;
		SQLExecutor db = new SQLExecutor();
		Statement stmt = null;
		...
		String sql = "" + "SELECT inv.billnum, " + "       date_format(inv.dateadded,'%m-%d-%Y') As DateAdded, "
				+ "       date_format(inv.DueDate,'%m-%d-%Y') As DueDate, " + "       inv.amountdue, "
				+ "       inv.memo, " + "       inv.status, " + "       inv.checkno, " + "       cv.NAME, "
				+ "       cv.clientvendorid " + "FROM   bca_bill AS inv " + "       INNER JOIN bca_clientvendor AS cv "
				+ "               ON cv.clientvendorid = inv.vendorid " + "WHERE  cv.companyid = '" + cId + "'"
				+ "       AND ( cv.status = 'U' " + "              OR cv.status = 'N' ) " + "       AND cv.deleted = 0"
				+ dateBetween;
	...
	public ArrayList getARGraphReport(String cId, HttpServletRequest request) {
		...
		try {
			...
			String sql_pie = "" + "SELECT clientvendorid, " + "       Sum(adjustedtotal) " + "FROM   bca_invoice "
					+ "WHERE  invoicetypeid IN ( 1, 3, 7 ) " + "       AND companyid = '" + cId + "'"
					+ "       AND invoicestatus = 0 " + "       AND ispaymentcompleted <> true "
					+ "GROUP  BY clientvendorid";

			String sql_bar = "" + "SELECT Sum(adjustedtotal)AS qty, "
					+ "       Date_format(bca_invoice.dateadded, '%b%y') AS t " + "FROM   bca_invoice "
					+ "WHERE  invoicetypeid IN ( 1, 3, 7 ) " + "       AND companyid = '" + cId + "'"
					+ "       AND invoicestatus = 0 " + "       AND ispaymentcompleted <> 1 "
			...
			while (rs1.next()) {
				AccountDto f = new AccountDto();
				String sql_vendor = ""
						+ "SELECT FirstName,LastName,Name FROM bca_clientvendor  WHERE  ClientVendorID = "
						+ rs1.getInt(1) + " AND Status='N'";
	...
	public ArrayList getIEGraphReport(String cId, HttpServletRequest request) {
		...
		try {
			...
			String sql_pie = "" + "SELECT clientvendorid, " + "       Sum(adjustedtotal) " + "FROM   bca_invoice "
					+ "WHERE  invoicetypeid IN ( 1, 3, 7 ) " + "       AND companyid = '" + cId + "' "
					+ "       AND invoicestatus = 0 " + "GROUP  BY clientvendorid";

			String sql_bar = "" + "SELECT Sum(adjustedtotal) AS qty, "
					+ "       Date_format(bca_invoice.dateadded, '%b%y') AS t " + "FROM   bca_invoice "
					+ "WHERE  invoicetypeid IN ( 1, 3, 7 ) " + "       AND companyid = '" + cId + "' "
					+ "       AND invoicestatus = 0 " + "GROUP  BY Date_format(bca_invoice.dateadded, '%b%y') "
					+ "ORDER  BY Str_to_date(Date_format(bca_invoice.dateadded, '%b%y'), '%M %d,%Y')";
	...
	public ArrayList getNWGraphReport(String cId, HttpServletRequest request) {
		...
		try {
			...
			String sql_bar = "" + "SELECT Sum(bca_payment.amount)                 AS qty, "
					+ "       Format(bca_payment.dateadded, 'mmm yy') AS t " + "FROM   (bca_category "
					+ "        INNER JOIN bca_categorytype " + "                ON bca_category.categorytypeid = "
					+ "       bca_categorytype.categorytypeid) " + "       INNER JOIN bca_payment "
					+ "               ON bca_category.categoryid = bca_payment.categoryid "
					+ "WHERE  bca_payment.companyid = '" + cId + "' "
					+ "       AND bca_categorytype.categorytypeid = 1973117447 " + "       AND isneedtodeposit = 0 "
					+ "       AND deleted = 0 " + "GROUP  BY Date_format(bca_payment.dateadded, '%b%y') "
			...
			// File(contextpath+"/IncomeExpencePie"+userId+".png").toPath());
			File fil2 = new File(contextpath + "/NetworthGraphBar" + userId + ".png");
			ChartUtilities.saveChartAsPNG(fil2, barChart, 600, 400);

			// pieChart = ChartFactory.createPieChart3D("IncomeExpence", dataset, true,
			// true, true);
			System.out.println(contextpath);
			/*
			 * File fil1 = new File(contextpath+"/IncomeExpencePie"+userId+".png");
	...
	public ArrayList getBvAGraphReport(String cId, HttpServletRequest request) {
		...
		try {
			...
			String sql_bar1 = "" + "SELECT Sum(pmt.amount)                 AS Amount1, "
					+ "       Format(pmt.dateadded, 'mmm yy') AS t " + "FROM   (bca_category AS cat "
					+ "        INNER JOIN bca_categorytype AS catType "
					+ "                ON cat.categorytypeid = catType.categorytypeid) "
					+ "       INNER JOIN bca_payment AS pmt " + "               ON cat.categoryid = pmt.categoryid "
					+ "WHERE  pmt.companyid = '" + cId + "' " + "       AND isneedtodeposit = 0 "
					+ "       AND deleted = 0 " + "       AND catType.categorytypeid = 1973117447 "
					+ "GROUP  BY Date_format(pmt.dateadded, '%b%y') "
					+ "ORDER  BY Str_to_date(Date_format(pmt.dateadded, '%b%y'), '%M %d,%Y')";
			String sql_bar2 = "" + "SELECT Sum(pmt.amount) AS Amount, " + "       NAME " + "FROM   bca_category AS cat "
					+ "       INNER JOIN bca_payment AS pmt " + "               ON cat.categoryid = pmt.categoryid "
					+ "WHERE  pmt.companyid = '" + cId + "' " + "       AND isneedtodeposit = 0 "
					+ "       AND deleted = 0 " + "GROUP  BY NAME";
	...
	public ArrayList eSaleSalesGraph(String cId, HttpServletRequest request) {
		...
		try {
			...
			stmt1 = con.createStatement();
			stmt2 = con.createStatement();
			String sql_bar = "" + "SELECT Sum(bca_cart.qty * bca_cart.unitprice) AS qty, "
					+ "       Date_format(bca_cart.dateadded, '%b%y')    AS t " + "FROM   (bca_cart "
					...
					+ "WHERE  bca_invoice.invoicetypeid = 1 " + "       AND bca_invoice.companyid = '" + cId + "' "
					+ "       AND invoicestatus = 0 " + "GROUP  BY Date_format(bca_invoice.dateadded, '%b%y') "
					+ "ORDER  BY Str_to_date(Date_format(bca_invoice.dateadded, '%b%y'), '%M %d,%Y' " + "          )";

			String sql_pie = "" + "SELECT * " + "FROM   (SELECT DISTINCT bca_iteminventory.inventorycode, "
					+ "                        Sum(bca_cart.qty * bca_cart.unitprice) AS qty, "
					+ "                        Sum(bca_cart.qty)                          AS "
					+ "                        totalQty, " + "                        bca_store.storename, "
					+ "                        bca_store.storetypename " + "        FROM   ((bca_cart "
					+ "                 INNER JOIN bca_invoice "
					+ "                         ON bca_cart.invoiceid = bca_invoice.invoiceid) "
					+ "                INNER JOIN bca_iteminventory "
					...
					+ "        WHERE  bca_invoice.invoicetypeid = 1 " + "               AND bca_invoice.companyid = '"
					+ cId + "' " + "               AND invoicestatus = 0 "
					+ "        GROUP  BY bca_iteminventory.inventorycode, " + "                  invoicetypeid, "
					+ "                  bca_store.storename, " + "                  bca_store.storetypename) AS t";

			/*
			 * while(rs1.next()) { AccountDto f = new AccountDto(); String sql_vendor = "" +
			 * "SELECT FirstName,LastName,Name FROM bca_clientvendor  WHERE  ClientVendorID = "
			 * +rs1.getInt(1) + " AND Status='N'";
			 * 
			 * rs2 = stmt.executeQuery(sql_vendor); while(rs2.next()) {
			 * f.setKey(rs2.getString("FirstName") + " " + rs2.getString("LastName")); }
			 * f.setValue(rs1.getDouble(2)); if(f.getKey() != null) {
			 * dataset.setValue(f.getKey(), f.getValue()); //remove this while apply latest
			 * code } else { dataset.setValue("", f.getValue()); } pieChartList.add(f); }
			 */
			DefaultPieDataset pieDataset = new DefaultPieDataset();
			String contextpath = request.getServletContext().getRealPath("/ChartReports");
			DefaultCategoryDataset dataset1 = new DefaultCategoryDataset();
	...
	public ArrayList getAccountPayableGraph(String cId, HttpServletRequest request) {
		...
		try {
			...
			String sql_bar = "" + "SELECT Sum(adjustedtotal)                             AS qty, "
					+ "       Date_format(bca_invoice.dateadded, '%b%y') AS t " + "FROM   bca_invoice "
					+ "WHERE  invoicetypeid = 9 " + "       AND companyid = '" + cId + "' "
					+ "       AND invoicestatus = 0 " + "GROUP  BY Date_format(bca_invoice.dateadded, '%b%y') "
					+ "ORDER  BY Str_to_date(Date_format(bca_invoice.dateadded, '%b%y'), '%M %d,%Y' " + "          )";
	...
	/**
	 * This method used to get the list of all the transaction done with the
	 ...
	 */
	public static ArrayList getAccountDetail(String companyID, String accountId, HttpServletRequest request) {
		...
		try {
			...
			sqlString = "select PayeeID,payerId,Amount,IsToBePrinted,PaymentId,date_format(DateAdded,'%Y-%m-%d') from bca_payment where CompanyID ="
					+ companyID + " and isNeedtoDeposit = false  and (PayerID=? OR PayeeID=?) order by DateAdded desc;";
	...
	/**
	 * This method is to get all the "payment" transactions list with a particular
	 * bank account.
	 * 
	 * @param addF
	 * @param request
	 * @return list of all the payment transactions done with the particular bank
	 *         account.
	 */
	public static ArrayList getPaymentAccountDetailList(String companyID, AddAccountForm addF,
		...
		try {
			...
			sqlString = "select payeeId,payerId,Amount,IsToBePrinted,PaymentId,date_format(DateAdded,'%m-%d-%Y') from bca_payment where CompanyID = "
					+ companyID
					+ " and isNeedtoDeposit = false and PayerID=? and (date_format(DateAdded,'%Y-%c-%e') between ? and ? )order by DateAdded desc;";
			pstmt = con.prepareStatement(sqlString);
	...
	/**
	 * This method is to get all the "deposit" transactions list with a particular
	 * bank account.
	 * 
	 * @param addF
	 * @param request
	 * @return list of all the deposit transactions done with the particular bank
	 *         account.
	 */
	public static ArrayList getDepositAccountDetailList(String companyID, AddAccountForm addF,
			HttpServletRequest request) {
		...
		try {
			...
			sqlString = "select payeeId,payerId,Amount,IsToBePrinted,PaymentId,date_format(DateAdded,'%m-%d-%Y') from bca_payment where CompanyID ="
					+ companyID
					+ " and isNeedtoDeposit = false and PayeeID=? and (date_format(DateAdded,'%Y-%c-%e') between ? and ? ) order by DateAdded desc";
			pstmt = con.prepareStatement(sqlString);
			pstmt.setInt(1, Integer.parseInt(addF.getBankId()));
			pstmt.setString(2, addF.getFrom());
			pstmt.setString(3, addF.getTo());
	 ...
	 */
	public static ArrayList getAccountCategories() {
		...
		try {
			...
			while (rs.next()) {
				accountCategories.add(new org.apache.struts.util.LabelValueBean(rs.getString("Name"),
						rs.getString("acctCategoryId")));
			}
		} catch (Exception e) {
	...
	public static ArrayList getBankList() {
		...
		try {
			...
			while (rs.next()) {
				bankList.add(
						new org.apache.struts.util.LabelValueBean(rs.getString("name"), rs.getString("accountId")));
			}
	 ...
	 */
	public static ArrayList getSubAccountsById(String id) {
		...
		con = db.getConnection();
		try {
			String sqlString = "select distinct accountId,Name from bca_account where acctCategoryId=" + id
					+ " and parentId=0 and active=1 order by Name asc";
			stmt = con.createStatement();
			rs = stmt.executeQuery(sqlString);
	 ...
	 * @return
	 */
	public static boolean addAccount(AddAccountForm af, HttpServletRequest request, String companyID) {
		...
		try {
			...
			if ("true".equals(af.getIsCategory())) {
				...
				query = "update bca_account set ParentID =?, isCategory =false,Description = ?, AcctTypeID = 2, AcctCategoryID = ?, CompanyID = "
						+ companyID
						+ ", Active = 1, CustomerStartingBalance = ?, CustomerCurrentBalance = ?, DateAdded = ? where AccountID = ?";
				pstmt = con.prepareStatement(query);
				...
				query = "update bca_payment set  Amount = ?, PaymentTypeID = -1 , PayerID = ?, PayeeID = ?, AccountID = 0 , ClientVendorID = 0 , CompanyID = "
						+ companyID
						+ ", IsToBePrinted = false , isNeedtoDeposit = false ,dateAdded=? where paymentID = ?";
				pstmt = con.prepareStatement(query);
				if ("".equals(af.getOpeningBalance()))
					pstmt.setString(1, "0");
				else
	 ...
	 * @return
	 */
	public static boolean deleteAccount(AccountDto af) {
	 ...
	 * @return
	 */
	public static AddAccountForm getAccountInfo(String accountId) {
	 ...
	 * @return
	 */
	public static int updateAccountInfo(AddAccountForm af) {
	 ...
	 * @return
	 */
	public static int deleteAccountTransaction(SaveAccountForm saf) {
	 ...
	 * @return
	 */
	public static int saveAccountTransaction(SaveAccountForm saf, HttpServletRequest request, String companyID) {
		...
		try {
			...
			sqlString = "update bca_payment set amount =?, PaymentTypeID = -1, PayerID =?, PayeeID =?, CompanyID = "
					+ companyID + ", DateAdded =? WHERE PaymentID =?";
			pstmt = con.prepareStatement(sqlString);
	 ...
	 * @return
	 */
	public static boolean saveReconciliationTransactions(AddAccountForm af, String paymentIds, String amounts,
			String revBal) {
	 ...
	 * @return
	 */
	public static ArrayList getPaymentCategoryDetailList(AddAccountForm addF, HttpServletRequest request,
			String companyID) {
		...
		try {
			...
			String sqlString = "select payeeId,payerId,Amount,IsToBePrinted,PaymentId,CategoryID,date_format(DateAdded,'%m-%d-%Y') from bca_payment where CompanyID = "
					+ companyID
					+ " AND isNeedtoDeposit = false and PayerID=? and (date_format(DateAdded,'%Y-%c-%e') between ? and ? )order by DateAdded desc;";
			pstmt = con.prepareStatement(sqlString);
			pstmt.setInt(1, Integer.parseInt(addF.getBankId()));
	 ...
	 * @return
	 */
	public static ArrayList getDepositCategoryDetailList(AddAccountForm addF, HttpServletRequest request,
			String companyID) {
	...
	public ArrayList getInvoiceDetail(String cId, HttpServletRequest request) {
		...
		try {
			sql += "SELECT invoiceid, " + "       ordernum, " + "       clientvendorid, " + "       balance, "
					+ "       termid, " + "       paymenttypeid, "
					+ "       date_format(DateAdded,'%m-%d-%Y')As DateAdded, " + "       dateconfirmed, "
					+ "       b.NAME, " + "       b.invoicetypeid, " + "       a.serviceid "
					+ "FROM   bca_invoice AS a " + "       LEFT JOIN bca_invoicetype AS b "
					+ "              ON ( a.invoicetypeid = b.invoicetypeid ) " + "WHERE  a.invoicetypeid IN ( 1, 4 ) "
					+ "       AND companyid = '" + cId + "'" + "       AND ( ispaymentcompleted = 0 ) "
					+ "       AND invoicestatus = 0 " + "ORDER  BY dateadded, " + "          invoiceid DESC";
			con = db.getConnection();
			stmt = con.createStatement();
			rs = stmt.executeQuery(sql);
			while (rs.next()) {
				...
				String cust = " SELECT firstname,lastname,termid " + " FROM bca_clientvendor" + " WHERE  companyid = '"
						+ cId + "'" + " AND clientvendorid = " + f.getCvId() + " AND status IN ( 'U', 'N' ) "
						+ " AND deleted = 0 ";
				stmt1 = con.createStatement();
				...
				if (rs1.next()) {
					...
					if (f.getTermDays() > 0) {
						...
						java.util.Date dueDate = dInfo.getDueDate(
								new java.util.Date(cusInfo.string2date(f.getDateAdded()).getTime()), f.getTermDays());
	...
	public ArrayList arCustomerDetail(String cId, HttpServletRequest request) {
		...
		try {
			...
			sql += "SELECT clientvendorid, " + "       NAME, " + "       firstname, " + "       lastname "
					+ "FROM   bca_clientvendor " + "WHERE  ( cvtypeid = 1 " + "          OR cvtypeid = 2 ) "
					+ "       AND status IN ( 'N', 'U' ) " + "       AND deleted = 0 " + "       AND companyid = '"
					+ cId + "'" + "ORDER  BY NAME";
			rs = stmt.executeQuery(sql);
			while (rs.next()) {
				InvoiceForm f = new InvoiceForm();
				...
				String total_invoice = "select count(*),sum(Balance) from bca_invoice where CompanyID='" + cId
						+ "' and ClientVendorID=" + rs.getInt(1);
	...
	public static ArrayList getBalanceSheetReport(String datesCombo, String fromDate, String toDate, String sortBy,
		...
		ArrayList<java.util.Date> selectedRange = new ArrayList<>();
		ArrayList<AccountDto> objList = new ArrayList<>();
		double totalSaving = 0.00, totalAccountReceivable = 0.00, totalCurrentAssets = 0.00,
				totalInventoryAssets = 0.00, totalOtherAssets = 0.00, toa, totalAssets = 0.00;
		...
		try {
			...
			request.setAttribute("BalanceSheetCurrentAssets.TotalCheckingSavings",
					new DecimalFormat("#0.00").format(totalSaving));
			request.setAttribute("BalanceSheetCurrentAssets_TotalCheckingSavings",
					new DecimalFormat("#0.00").format(totalSaving));
			request.setAttribute("BalanceSheetCurrentAssets.AccountsReceivable",
					new DecimalFormat("#0.00").format(totalAccountReceivable));
			request.setAttribute("BalanceSheetCurrentAssets_AccountsReceivable",
					new DecimalFormat("#0.00").format(totalAccountReceivable));
			request.setAttribute("BalanceSheetCurrentAssets.TotalAccountsReceivable",
					new DecimalFormat("#0.00").format(totalAccountReceivable));
			request.setAttribute("BalanceSheetCurrentAssets_TotalAccountsReceivable",
					new DecimalFormat("#0.00").format(totalAccountReceivable));
			request.setAttribute("BalanceSheetCurrentAssets.TotalCurrentAssets",
					new DecimalFormat("#0.00").format(totalCurrentAssets));
			request.setAttribute("BalanceSheetCurrentAssets_TotalCurrentAssets",
					new DecimalFormat("#0.00").format(totalCurrentAssets));
			request.setAttribute("BalanceSheetOtherAssets.Inventory",
					new DecimalFormat("#0.00").format(totalInventoryAssets));
			request.setAttribute("BalanceSheetOtherAssets_Inventory",
					new DecimalFormat("#0.00").format(totalInventoryAssets));
			...
			request.setAttribute("BalanceSheetTotalLiabilitiesAndStockholdersEquity",
	...
	public static void getCashFlowReport(String datesCombo, String fromDate, String toDate, String sortBy, String cId,
			HttpServletRequest request, AccountDto form) {
		Connection con = null;
		SQLExecutor db = new SQLExecutor();
		Statement stmt1 = null, stmt2 = null, stmt3 = null, stmt4 = null, stmt5 = null, stmt6 = null, stmt7 = null,
				stmt8 = null, stmt9 = null, stmt10 = null, stmt11 = null, stmt12 = null, stmt13 = null, stmt14 = null;
		ResultSet rs1 = null, rs2 = null, rs3 = null, rs4 = null, rs5 = null, rs6 = null, rs7 = null, rs8 = null,
				rs9 = null, rs10 = null, rs11 = null, rs12 = null, rs13 = null, rs14 = null;
		con = db.getConnection();
		String dateBetween = "", sql1 = "";
		DateInfo dInfo = new DateInfo();
		CustomerInfo cInfo = new CustomerInfo();
		...
		try {
			...
			sql1 = "SELECT SUM(CustomerStartingBalance)as accountBalance  FROM bca_account WHERE AcctTypeID = 2 AND Active = 1 AND CompanyID='"
					+ cId + "'" + dateBetween;
			// sql1 ="select * from bca_account where CompanyID = '" + cId + "'" + " AND
			// AcctCategoryID in(1,2)";
			rs1 = stmt1.executeQuery(sql1);
			if (rs1.next()) {
				// AccountDto a = new AccountDto();
			...
			String sql2 = "SELECT SUM(CustomerOpenDebit) as customerBalance FROM bca_clientvendor WHERE CompanyID ='"
					+ cId + "' AND Status IN ('U', 'N' )  AND (Deleted = 0 OR Active = 1) AND CvTypeID IN(1,2)"
					+ dateBetween;
			rs2 = stmt2.executeQuery(sql2);
			if (rs2.next()) {
				AccountDto f = new AccountDto();
			...
			String sql3 = "SELECT SUM(VendorOpenDebit) as vendorBalance  FROM  bca_clientvendor WHERE CompanyID ='"
					+ cId + "' AND Status IN ('U', 'N' )  AND (Deleted = 0 OR Active = 1) AND CvTypeID IN(3)"
					+ dateBetween;
			rs3 = stmt3.executeQuery(sql3);
			if (rs3.next()) {
			...
			// AccountDto account = getAccount(56934,cId);
			/*
			 * if(account != null) { totalOtherAssets = 0.00; } toa = totalInventoryAssets +
			 * totalOtherAssets; totalAssets = totalCurrentAssets + toa;
			 */

			String sql4 = "SELECT SUM(Balance) FROM bca_invoice  WHERE CompanyID='" + cId
					+ "'  AND InvoiceTypeID= 1  AND IsPaymentCompleted = 0  AND InvoiceStatus <>1" + dateBetween;
			rs4 = stmt4.executeQuery(sql4);
			if (rs4.next()) {
				invoiceBalance = rs4.getDouble(1);
			}

			String sql5 = "Select sum(UpfrontAmount) as upfrontAmt  FROM bca_invoice  WHERE CompanyID='" + cId
					+ "'  AND InvoiceTypeID= 1  AND IsPaymentCompleted = 0  AND InvoiceStatus <>1" + dateBetween;
			rs5 = stmt5.executeQuery(sql5);
			if (rs5.next()) {
				amt = rs5.getDouble(1);
			...
			String sql6 = "SELECT SUM(payment.Amount) as paidAmount FROM bca_payment as payment,bca_clientvendor as cv   WHERE  payment.ClientVendorID = cv.ClientVendorID AND  payment.InvoiceID = -1 AND payment.RmaNo <= 0  AND payment.ClientVendorID > 0 AND payment.Deleted=0 AND cv.CVTypeID IN (1,2) AND payment.CompanyID= '"
					+ cId + "' AND cv.Status IN ('U', 'N' )AND (cv.Deleted = 0 OR cv.Active = 1)"
					+ dateBetween.replaceAll("DateAdded", "payment.DateAdded");
			rs6 = stmt6.executeQuery(sql6);
			if (rs6.next()) {
				paidOpeningBalanceOfCustomer = rs6.getDouble(1);
			}

			String sql7 = "SELECT SUM(PaidAmount) as netCash FROM bca_invoice WHERE CompanyID='" + cId
					+ "' AND InvoiceTypeID= 1" + dateBetween;
			String sql8 = "SELECT SUM(PaidAmount) as netCash FROM bca_invoice WHERE CompanyID='" + cId
					+ "' AND InvoiceTypeID= 2 AND InvoiceStatus <> 1" + dateBetween;
			String sql9 = "SELECT SUM(AmountPaid) as netCash FROM bca_bill WHERE CompanyID='" + cId
					+ "' AND BillType = 0" + dateBetween;
			...
			String sql10 = "SELECT SUM(Balance) FROM bca_invoice WHERE CompanyID='" + cId
					+ "' AND InvoiceTypeID= 2 AND IsPaymentCompleted = 0 AND InvoiceStatus <>1" + dateBetween;
			rs10 = stmt10.executeQuery(sql10);
			if (rs10.next()) {
				purchaseBalance = rs10.getDouble(1);
			}

			String sql11 = "SELECT SUM(payment.Amount) as paidAmount FROM bca_payment as payment,bca_clientvendor as cv   WHERE  payment.ClientVendorID = cv.ClientVendorID AND  payment.InvoiceID = -1 AND payment.RmaNo <= 0  AND payment.ClientVendorID > 0 AND payment.Deleted=0 AND cv.CVTypeID IN (3) AND payment.CompanyID= '"
					+ cId + "' AND cv.Status IN ('U', 'N' )AND (cv.Deleted = 0 OR cv.Active = 1)"
					+ dateBetween.replaceAll("DateAdded", "payment.DateAdded");
			...
			String sql12 = "SELECT  SUM(Balance) FROM bca_bill Where BillType =0 AND Status <> 1 AND CompanyID='" + cId
					+ "'" + dateBetween;
			...
			String sql13 = "SELECT SUM(Balance) FROM bca_bill Where BillType =1 AND Status <> 1 AND CompanyID='" + cId
					+ "'" + dateBetween;
			...
			// String sql14 = "SELECT Sum(a.Amount) FROM bca_payment as a,bca_invoice as b
			// WHERE a.Deleted = 1 AND a.InvoiceID = b.InvoiceID AND b.InvoiceStatus=1 AND
			// a.CompanyID='"+cId+"'" + dateBetween.replaceAll("DateAdded", "a.DateAdded");
			String sql14 = "SELECT Sum(a.amount) " + "FROM   bca_payment AS a, " + "       bca_invoice AS b "
					+ "WHERE  a.deleted = 1 " + "       AND a.invoiceid = b.invoiceid "
					+ "       AND b.invoicestatus = 1 " + "       AND a.companyid = '" + cId + "'"
			...
			double aPBalace = (vendorsOpeningBalance - paidOpeningBalanceOfVendor) + purchaseBalance + billBalance
					- creditBalance;
			...
			/*
			 * total = liability + equity; double amt = total-totalAssets; if(amt < 0){ amt
			 * = amt * (-1); }
			 */

			/*
			 * request.setAttribute("BalanceSheetCurrentAssets.TotalCheckingSavings", new
			 * DecimalFormat("#0.00").format(totalSaving));
			 * request.setAttribute("BalanceSheetCurrentAssets.AccountsReceivable", new
			 * DecimalFormat("#0.00").format(totalAccountReceivable));
			 * request.setAttribute("BalanceSheetCurrentAssets.TotalAccountsReceivable", new
			 * DecimalFormat("#0.00").format(totalAccountReceivable));
			 * request.setAttribute("BalanceSheetCurrentAssets.TotalCurrentAssets", new
			 * DecimalFormat("#0.00").format(totalCurrentAssets));
			 * request.setAttribute("BalanceSheetOtherAssets.Inventory", new
			 * DecimalFormat("#0.00").format(totalInventoryAssets));
			 * request.setAttribute("BalanceSheetOtherAssets.TotalOtherAssets", new
			 * DecimalFormat("#0.00").format(toa));
			 * request.setAttribute("BalanceSheetTotalAssets", new
			 * DecimalFormat("#0.00").format(totalAssets));
			 * request.setAttribute("BalanceSheetTotalLiabilities", new
			 * DecimalFormat("#0.00").format(liability));
			 * request.setAttribute("BalanceSheetTotalStockholdersEquity", new
			 * DecimalFormat("#0.00").format(equity));
			 * request.setAttribute("BalanceSheetTotalLiabilitiesAndStockholdersEquity", new
			 * DecimalFormat("#0.00").format(total));
			 * request.setAttribute("BalanceSheetAdjustAmount", new
			 * DecimalFormat("#0.00").format(amt));
			 */
			...
			request.setAttribute("CashFlowStatement.NetCashProvidedByOA",
					new DecimalFormat("#0.00").format(netOABalance));
			request.setAttribute("CashFlowStatement.OBE", new DecimalFormat("#0.00").format(netCashProvidedByFA));
			request.setAttribute("CashFlowStatement.NetCashProvidedByFA",
					new DecimalFormat("#0.00").format(netCashProvidedByFA));
			request.setAttribute("CashFlowStatement.NetCashIncreasedByPeriod",
					new DecimalFormat("#0.00").format(netCashIncreasedByPeriod));
			request.setAttribute("CashFlowStatement.CashAtTheEndOfPeriod",
					new DecimalFormat("#0.00").format(cashAtTheEndOfPeriod));
			...
			request.setAttribute("CashFlowStatement_NetCashProvidedByOA",
					new DecimalFormat("#0.00").format(netOABalance));
			request.setAttribute("CashFlowStatement_OBE", new DecimalFormat("#0.00").format(netCashProvidedByFA));
			request.setAttribute("CashFlowStatement_NetCashProvidedByFA",
					new DecimalFormat("#0.00").format(netCashProvidedByFA));
			request.setAttribute("CashFlowStatement_NetCashIncreasedByPeriod",
					new DecimalFormat("#0.00").format(netCashIncreasedByPeriod));
			request.setAttribute("CashFlowStatement_CashAtTheEndOfPeriod",
					new DecimalFormat("#0.00").format(cashAtTheEndOfPeriod));
	...
	public static AccountDto getAccount(int accountId, String cId) {
		Connection con = null;
		SQLExecutor db = new SQLExecutor();
