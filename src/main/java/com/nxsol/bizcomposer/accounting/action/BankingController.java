...
//		ReceivableLIst rl = new ReceivableListImpl();
		...
		if (action.equals("showTransaction")) {
			/*
			 * String ids = request.getParameter("Ac"); int accountIds =
			 * Integer.parseInt(ids); request.getSession().setAttribute("selectedAccountId",
			 * accountIds); TblAccount accounts = rl.getAccountById(accountIds);
			 * ArrayList<TblPaymentDto> payments = rl.getPaymentsForBanking(accounts);
			 * request.getSession().setAttribute("payMentList", payments);
			 * request.getSession().setAttribute("selectedAccount", accounts); forward =
			 * "success1";
			 */
		}
