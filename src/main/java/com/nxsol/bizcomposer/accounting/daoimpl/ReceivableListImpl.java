...
//			rs = stmt.executeQuery(sql);
			...
			for (BcaAccount bca_account : accounts) {
				...
				account.setLastCheckNo(bca_account.getLastCheck() != null ? bca_account.getLastCheck() : -1);
		...
		try {
			...
			if (receivableListBean.getPoNum() == 0) {
				...
				if (receivableListBean.getInvoiceID() < 0) {
					...
					i = bcaInvoiceRepository.updateInvoiceByInvoiceId(paymentType,
							receivableListBean.getBankAccountID(), bcaCategory, paidAmount, balance, clientvendor,
							receivableListBean.getMemo().trim(), receivableListBean.getInvoiceID(), company);
				}
...
//				+ " WHERE InvoiceID=" + invoice.getInvoiceID() + " AND CompanyID=" + ConstValue.companyId;
		String sql = " UPDATE bca_invoice SET " + " IsPaymentCompleted=" + (paymentCompleted == true ? 1 : 0) + ","
				+ " PaidAmount =" + paidAmount + "," + " Balance=" + invoice.getBalance() + "," + " AdjustedTotal="
				+ invoice.getAdjustedTotal() + "," + " CategoryID=" + invoice.getCategoryID() + "," + " ClientVendorID="
				+ invoice.getCvID() + "," + " invoiceStatus = " + invoice.getInvoiceStatus() + " WHERE InvoiceID="
				+ invoice.getInvoiceID() + " AND CompanyID=" + ConstValue.companyId;
		try {
...
//					priority + 1, company, payment.getId());
			...
			BcaPayment bcaPayment = bcaPaymentRepository.findByPaymentIdAndCompany_CompanyId(payment.getId(),
					Long.valueOf(ConstValue.companyId));
...
//		ResultSet rs = null;
		...
		String sql = "SELECT INV.InvoiceID, INV.PONum, INV.ClientVendorID , INV.CompanyID,INV.InvoiceTypeID "
				...
				+ " AND bca_clientvendor.Status IN ('N', 'U')" + " AND bca_clientvendor.CompanyID ="
				+ ConstValue.companyId + " AND ( INV.AdjustedTotal > (SELECT Sum(bca_payment.Amount)"
...
//	New Functions for Banking Page Bank Account Tree starts from here 20240522
	@Override
	public ArrayList<TblAccountCategory> getBankAccountsTree(ArrayList<TblAccountCategory> categoryList) {
		for (TblAccountCategory category : categoryList) {
			ArrayList<TblAccount> accounts = getBankAccountsOfCategory(category.getAccountCategoryID());
	...
	private ArrayList<TblAccount> getBankAccountsOfCategory(int accountCategoryId) {
		ArrayList<TblAccount> tempBankAccounts = new ArrayList<>();

		for (TblAccount bankAccount : bankAccountsInFundTransfer) {
	...
	public ArrayList<TblAccount> getAllAccountsFromBankAccountsTree(ArrayList<TblAccountCategory> bankAccountTreeWithCat) {
	    ArrayList<TblAccount> allAccounts = new ArrayList<>();

	    for (TblAccountCategory category : bankAccountTreeWithCat) {
	        if (category.getAccounts() != null) {
	            allAccounts.addAll(category.getAccounts());
	        }
	    }
...
//	New Functions for Banking Page Bank Account Tree ends here 20240522
...
//		sql.append(" ORDER BY pay.Priority DESC"); changed on 20240301
		sql.append(" ORDER BY pay.PaymentID DESC");
...
//			rs = stmt.executeQuery(Sql);
			...
			for (BcaBillDto dto : billDtos) {
				...
				vDetail.setServiceID(dto.getServiceId() != null ? dto.getServiceId() : 0);
				boolean status = Boolean.TRUE.equals(dto.getIsMemorized());
				...
				int bStatus = Integer.valueOf(dto.getStatus());
				if (bStatus == 1)
					billStatus = "Paid";
				if (bStatus == 2)
					billStatus = "Schedule";
...
//			List<Object[]> lists = bcaPaymentRepository.findPaidBillListsPayment(new Long(ConstValue.companyId));
			...
			for (BcaPaymentDto dto : objectToBcaPaymentDto) {
				...
				payment.setTotalAmount(totaAmount);
				payment.setPaymentTypeID(dto.getPaymentTypeId() != null ? dto.getPaymentTypeId() : 0);
				/*
				 * payment.setPaymentTypeName(tblPaymentLoader.getPaymentTypeName(payment.
				 * getPaymentTypeID()));
				 */
				...
				payment.setServiceId(dto.getServiceId() != null ? dto.getServiceId() : 0);
				...
				payment.setCategoryName(dto.getCategoryName());
...
//			List<Object[]>list=bcaClientvendorRepository.findPayBillsLists(new Long(ConstValue.companyId), format);
	...
	public void insertNewBill(TblVendorDetail vDetail) throws ParseException {
		...
		String sql = "INSERT into bca_bill(VendorId,PayerID,CompanyID,DateAdded,DueDate,AmountDue,Status,Memo,BillType,Balance,NextDate,CategoryID,ServiceID) Values("
				...
				+ "," + "'" + DueDate1 + "'" + "," + vDetail.getCategoryID() + "," + null + ")";
...
//		}
	...
	@Override
	public ArrayList<ReceivableListDto> getAllInvoicesForBillingBoardPaidListWithSearchOption(Date from, Date to,
		...
		String sql = "SELECT OrderNum, Total, inv.TermID, Memo, Note, inv.InvoiceID, ServiceID, InvoiceTypeID, JobCategoryID, "
				...
				+ "FROM " + billingBoard_table + " as inv " + "LEFT JOIN bca_term AS term ON inv.TermID = term.TermID "
				...
				+ "ON inv.InvoiceID = p.InvoiceID " + "WHERE inv.CompanyID = " + ConstValue.companyId + " "
				+ "AND NOT (invoiceStatus = 1) " + "AND inv.IsPaymentCompleted = 1 " + "AND cv.Status IN ('U','N')";
		...
		if (overdueDays > 0) {
			sql += " AND (DATEDIFF(CURDATE(), DATE_ADD(inv.DateAdded, INTERVAL term.Days + " + overdueDays
					+ " DAY)) = 0)";
		}
...
//				dto.setDateAdded(DateHelper.convertDateToOffsetDateTime(dateAdded));
