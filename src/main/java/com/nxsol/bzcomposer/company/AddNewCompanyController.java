...
public class AddNewCompanyController {
	...
	public ModelAndView AddNewCompany(CompanyInfoDto companyInfoFormInput, HttpServletRequest request,
...
//			AddNewCompanyDAO dao = new AddNewCompanyDAO();
			...
			dao.getBusinessType(request, companyInfoForm);
		...
		if (action.equalsIgnoreCase("createNewCompany2")) {
			...
			if (isDuplicateName == true) {
				...
				forward = "createNewCompany";
			} else if (companyInfoFormInput.getCompanyName() == null
					|| companyInfoFormInput.getCompanyName() == "" && isDuplicateName == false) {
				ActionErrors e = new ActionErrors();
...
//			}
		...
		if (action.equalsIgnoreCase("createNewCompany10")) {
			...
			if (companyInfoForm.getCompanyName() != null
					|| companyInfoForm.getCompanyName() != "" && isDuplicateName == false) {
		...
		if (action.equalsIgnoreCase("addCustomer")) {
			Gson gson = new Gson();
			Type listType = new TypeToken<List<CompanyInfoDto>>() {
			}.getType();
