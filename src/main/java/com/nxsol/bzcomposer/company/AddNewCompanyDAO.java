...
public class AddNewCompanyDAO {
	...
	public ArrayList<CompanyInfoDto> getBusinessType(HttpServletRequest request, CompanyInfoDto form) {
		ArrayList<CompanyInfoDto> listPOJOs = new ArrayList<>();
		try {
			// Fetch all active business types ordered by business name
			List<BcaBusinesstype> businessTypes = businesstypeRepository.findByActiveOrderByBusinessNameAsc(1);

			for (BcaBusinesstype bt : businessTypes) {
				CompanyInfoDto pojo = new CompanyInfoDto();
				pojo.setBusinessName(bt.getBusinessName());
				pojo.setBusinessTypeId(bt.getBusinessTypeId());
				listPOJOs.add(pojo);
			}
		} catch (Exception e) {
			Loger.log("Exception in getBusinessType: " + e.toString());
		}
...
//	}
	 ...
	 * SQLExecutor db = new SQLExecutor(); ocon = db.getConnection();
	 * 
	 * int newCompanyid=0;
	 * 
	 * try {
	 ...
	 * 
	 * } catch (Exception e) { Loger.log(e.toString()); } finally { try { // close
	 * connection
	 * 
	...
	public void insertUpfrontItem(int companyId, boolean isUpdate) {
		...
		try {
			...
			sSqlItemCat = " bca_iteminventory" + " (CompanyID,ParentID,InventoryName,InventoryCode," + "isCategory,"
					+ "ItemTypeID,Active)" + " values (" + companyId + "," + "0" + "," + "'Upfront Deposit'" + ","
					+ "'Upfront Deposit'" + "," + "1" + "," + // IsCategory
					" " + 6 + "," + // ItemTypeID = 6
					"1" + " ) ";
	...
	public ArrayList getQueriesForJPanel40(CompanyInfoDto form) {
		...
		if (!vPaymentType_Name.isEmpty()) {
			for (i = 0; i < vPaymentType_Name.size(); i++) {
				 ...
				 * handle exception Loger.log(e.toString()); } }
				 */
			}
		}
		if (!vCCType_Name.isEmpty()) {
		...
		if (!vShipCarrier_Name.isEmpty()) {
			for (i = 0; i < vShipCarrier_Name.size(); i++) {
				 ...
				 * TODO: handle exceptio Loger.log(e.toString()); } }
				 */
			}
		}
