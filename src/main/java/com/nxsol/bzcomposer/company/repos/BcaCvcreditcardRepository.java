...
public interface BcaCvcreditcardRepository extends JpaRepository<BcaCvcreditcard, Integer> {
	...
	@Query(value = "update  bca_cvcreditcard set  CardNumber = ?, CardExpMonth = ?, CardExpYear = ?, CardCW2 = ?, CardHolderName = ? CardBillingAddress = ?, CardBillingZipCode = ?, Active = 1,"
			+ " DateAdded = ?,CCTypeID = ? where CreditCardID = ? and clientvendorid = ?", 
	  nativeQuery = true)
	int updateByCreditCardIdAndClientVendorId(String CardNumber, String CardExpMonth, String CardExpYear, String CardCW2, String CardHolderName, String CardBillingAddress, String CardBillingZipCode,
	...
	List<BcaCvcreditcard> findByClientVendorId(String cvID);
