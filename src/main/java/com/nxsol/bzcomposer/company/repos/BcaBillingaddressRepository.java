...
public interface BcaBillingaddressRepository extends JpaRepository<BcaBillingaddress, Integer> {
	...
	@Query("select bi from BcaBillingaddress bi where bi.clientVendor.clientVendorId = :clientVendorId and bi.status = :status")
		BcaBillingaddress findByClientVendorIdAndStatus(@Param("clientVendorId")Integer clientVendorId , @Param("status")String status);
	...
	int updateStatusByClientVendorIdAndStatusIn(@Param("status")String status, @Param("clientVendorId")Integer clientVendorId,@Param("statusList") List<String> statusList);
