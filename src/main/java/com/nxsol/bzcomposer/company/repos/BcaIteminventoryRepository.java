...
public interface BcaIteminventoryRepository extends JpaRepository<BcaIteminventory, Integer> {
	...
	int updateSalesPriceByInventoryIdAndCompanyId(@Param("salePrice") Double salePrice,
			@Param("inventoryId") Integer inventory, @Param("companyId") Long companyId);
	...
	int updateInventoryNameByInventoryIdAndCompanyId(@Param("inventoryName") String inventoryName,
			@Param("inventoryId") Integer inventory, @Param("companyId") Long companyId);
	...
	List<BcaIteminventory> findByCompanyIdAndActiveAndItemTypeIdNot(@Param("companyId") Long companyId,
			@Param("active") Integer active, @Param("itemTypeId") Integer itemTypeId);
	...
	List<BcaIteminventory> findByCompanyIdAndActiveAndItemTypeNotAndInventoryId(@Param("companyId") Long companyId,
			@Param("active") Integer active, @Param("itemTypeId") Integer itemTypeId,
	...
	List<BcaIteminventory> findByCompany_CompanyIdAndActiveAndParentIdLessThan(long companyId, int active,
			int parentId);

	List<BcaIteminventory> findByItemTypeIdAndActiveAndParentId(int itemTypeId, int active, int parentId);

	@Query("FROM BcaIteminventory Where CompanyID =:cid and parentId!= 0")
	List<BcaIteminventory> getAllProduct(String cid);
